    ...
    return (
      <View style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={[!updatedCountry.currencies.length && { flex: 1 }]}>
          ...
          >
            {!updatedCountry.currencies.length && <CenterMessage message="No additional currencies for this country!" />}
            {updatedCountry.currencies.map((currency, index) => (
              <View key={index} style={styles.currencyContainer}>
                <Text style={styles.currencyName}>{currency.name}</Text>
                <Text style={styles.currencyUsed}>{currency.info}</Text>
              </View>
            ))}
          </View>
        </ScrollView>
